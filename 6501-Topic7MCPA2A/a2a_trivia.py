...
USAGE:
  ...
  Options:
    ...
    --smart-route     Instead of broadcasting to all agents, route each question
                      to the top 5 agents whose Agent Card descriptions best
                      match the question (TF-IDF cosine similarity)
    --top N           Number of agents to route to in smart-route mode (default: 5)
...
def rank_agents_for_question(question: str, agents: list[dict], top_n: int = 5) -> list[tuple[dict, float]]:
    """
...
def run_tournament(registry_url: str, rounds_per_category: int | None,
                   ...
                   smart_route: bool = False, top_n: int = 5):
    ...
    mode_label = f"smart-route (top {top_n})" if smart_route else "broadcast"
    print(f"🎯 Tournament: {total} questions across {len(TRIVIA_QUESTIONS)} categories")
    print(f"📡 Mode: {mode_label}\n")

    # If smart routing, fetch full agent list once
    all_agents = []
    if smart_route:
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top", type=int, default=5,
                        help="Number of agents to route to in smart-route mode (default: 5)")
